...
const DevicePositionPage: React.FC<InfoProps> = ({ deviceInfo, refresh }) => {
  ...
  const getAddr = (point, flag) => {
    // 检查是否已有地址（模拟模式）
    const existingAddr = deviceInfo?.address || pos || '模拟地址';
    
    // 如果百度地图未加载或处于模拟模式，直接使用现有地址
    if (!window.BMap || !point) {
      setLocal('addr', existingAddr);
      if (flag === 'loc') {
        devicePosHandle({ 
          address: existingAddr,
          point: { lng: point?.lng || 116.403963, lat: point?.lat || 39.915119 }
        }, 'loc');
      }
      return;
    }
    ...
    geocoderRef.current?.getLocation(point, (GeocoderResult) => {
      if (!GeocoderResult || !GeocoderResult.address) {
        // 解析失败时使用默认地址，不显示错误
        const fallbackAddr = existingAddr;
        setLocal('addr', fallbackAddr);
        if (flag === 'loc') {
          devicePosHandle({ 
