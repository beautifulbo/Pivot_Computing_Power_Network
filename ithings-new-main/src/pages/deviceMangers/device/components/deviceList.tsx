...
const DeviceList: React.FC<Props> = ({ productInfo }) => {
  ...
  const queryPage = async (
    ...
    // 使用模拟数据
    let deviceList = mockDeviceListData.data.list as any[];
    
    // 简单的筛选逻辑
    if (params.productID) {
      deviceList = deviceList.filter((d: any) => d.productID === params.productID);
    }
    if (params.deviceName) {
      deviceList = deviceList.filter((d: any) => 
        d.deviceName.includes(params.deviceName!)
      );
    }
    if (params.isOnline && params.isOnline !== 0) {
      deviceList = deviceList.filter((d: any) => d.isOnline === params.isOnline);
    }
    
    return {
      data: deviceList as DEVICE_INFO[],
