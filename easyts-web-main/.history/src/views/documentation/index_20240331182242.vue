...
<div style="margin-top: 80px;">
            ...
            <div id="plotly-mps" style="margin-top: 80px;"></div>  
          <el-divider>平均丢包率预测结果</el-divider>
          <div id="plotly-diubao" style="margin-top: 80px;"></div>  
          <el-divider>时延预测结果</el-divider>
          <div id="plotly-delay" style="margin-top: 80px;"></div> 
        </div> 
      <el-divider />
        <!-- </el-col> -->
      </el-col>
...
console.log(csvData);
this.mpsplotGraph(csvData,this.predictday);
...
var csvData = d3.csvParse(res.data)
console.log(csvData);
this.diubaoplotGraph(csvData,this.predictday);
...
this.delayplotGraph(csvData,this.predictday);
...
if(predictday===7||predictday===14){
  daysToPlot=predictday
      ...
      const data = [
      {
            x: timeSeries.slice(0, (14+daysToPlot) * 24), // 过去14天的时间序列
            y: gtValues.slice(0, (14+daysToPlot) * 24), // 过去14天的真实流量
            mode: 'lines',
            name: '真实带宽',
            line: { width: 4 }
        },
        {
            x: timeSeries.slice(14 * 24, (14 + daysToPlot) * 24), // 未来7天的时间序列
            y: pdValues.slice(14 * 24, (14 + daysToPlot) * 24), // 未来7天的预测流量
            mode: 'lines',
            name: '预测带宽',
      ...
      const layout = {
        ...
        yaxis: { title: '带宽（mps）', titlefont: { size: 18 }, tickfont: { size: 16 } },
