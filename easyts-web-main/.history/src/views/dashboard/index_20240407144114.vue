<template>
  ...
  <el-menu :router="true" :default-active="this.$route.path" active-text-color='#2E5BFF' mode="horizontal" >
  <el-menu-item index="/dashboard" style="font-size: 14px;">流量预测</el-menu-item>
 
  <el-menu-item index="/documentation/index">质量预测</el-menu-item>
</el-menu>
      <el-row>
        <el-col :span="8" >
          <div style="display: flex; align-items: center; margin-top:20px;margin-right: 40px;justify-content: center; font-size: 14px;">
    模型介绍
  </div>
...
<el-card class="box-card" style="margin-top:20px;margin-left: 20px;width: 80%;height: 80%;">
  <div style="font-size: 14px; text-align: center;">
    QM-RGNN是一个新型QoS预测框架，它利用图卷积网络（GCN）和多层感知机（MLP）构成的编码器-解码器模型QM-GNN，将QoS数据作为图输入进行高效预测。该框架通过引入残差模块处理QoS数据的稀疏性和波动性，采用动态自适应采样比例降低采样成本，并设计在线学习模式以减少持续训练成本。在实际应用中，QM-RGNN显著提升了QoS预测的精确度，实现了显著的均方根误差（RMSE）降低和训练及采样成本的大幅度降低。  </div>
        ...
        <el-col  :span="4" >
          <el-button type="primary" @click="onClick" style="margin-top:20px;font-size:14px;background-color: #2E5BFF;">预测</el-button>
        </el-col>        
        <div >
...
value:7,
  ...
  },
  {
    ...
    methods: {
      onClick() {
        this.showtable=true
        axios.get('../static/liuliang_all_predictions.csv').then(res => {
...
//         xaxis: {
//     type: 'date',
//     tickmode: 'auto',  // 设置刻度模式为自动
//     tickformat: '%b %d',  // 设置日期显示格式为月份和日期
//     title: '日期',  // x轴标题
//     titlefont: { size: 18 },  // 标题字体大小
//     tickfont: { size: 16 }  // 刻度字体大小
// },
        yaxis: { title: '流量（Tbps）', titlefont: { size: 18 }, tickfont: { size: 16 } },
