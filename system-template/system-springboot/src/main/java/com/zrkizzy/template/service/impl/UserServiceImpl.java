...
public class UserServiceImpl implements IUserService {
     ...
     */
    @Override
    public Result login(String username, String password, String code, String key, HttpServletRequest request) {
        // 1. -------------------- 判断验证码 --------------------
        // 根据key从Redis中获取验证码（支持多用户并发）
        String redisKey = KAPTCHA;
        if (key != null && !key.trim().isEmpty()) {
            redisKey = KAPTCHA + ":" + key;
        }
        String kaptcha = (String) redisTemplate.opsForValue().get(redisKey);

        // 临时调试日志
        System.out.println("========== 验证码调试 ==========");
        System.out.println("请求的key: " + key);
        System.out.println("Redis key: " + redisKey);
        System.out.println("用户输入验证码: [" + code + "]");
        System.out.println("Redis验证码: [" + kaptcha + "]");
        System.out.println("验证码为空: " + StringUtils.isEmpty(kaptcha));
        System.out.println("验证码比较结果: " + (kaptcha != null ? kaptcha.equalsIgnoreCase(code) : "null"));
        System.out.println("===============================");
