...
public class UserInfoServiceImpl implements IUserInfoService {
    ...
    public Result updateUserInfo(UserInfoDTO userInfoDTO) {
        ...
        if (count > 0) {
            String successMessage = "信息更新成功";
            log.info("=============== 调试信息 ===============");
            log.info("返回消息: {}", successMessage);
            log.info("消息字节数组(UTF-8): {}", java.util.Arrays.toString(successMessage.getBytes(java.nio.charset.StandardCharsets.UTF_8)));
            log.info("消息字节数组(ISO-8859-1): {}", java.util.Arrays.toString(successMessage.getBytes(java.nio.charset.StandardCharsets.ISO_8859_1)));
            Result result = Result.success(successMessage);
            log.info("Result对象: {}", result);
            log.info("======================================");
            return result;
        }
