  ...
  <div class="module-container">
    ...
    <el-card class="module-card" v-if="!showIframe">
      ...
      <div class="module-header">
        <div class="module-icon" :style="{ background: '#9333EA' }">
          <i class="el-icon-data-line"></i>
        </div>
        <h1 class="module-title">智能流量预测</h1>
        <p class="module-subtitle">网络流量智能预测与分析平台</p>
      </div>
      ...
      <div class="module-section">
        ...
        <p class="section-content">
          基于多模型融合的网络流量预测技术,支持对历史流量数据的分析和未来流量的智能预测,实现短期、中长期和长期的高精度预测,帮助用户优化资源配置和网络调度。
        </p>
      </div>
      ...
      <div class="module-section">
        ...
        <ul class="features-list">
          <li>
            <el-tag type="primary" effect="plain">多模型融合预测</el-tag>
            <span>集成AnaNet、DynEformer、QM-RGNN等先进模型,提高预测准确度</span>
          </li>
          <li>
            <el-tag type="primary" effect="plain">多时间尺度预测</el-tag>
            <span>支持短期(≥90%)、中长期(≥85%)和长期(≥80%)预测</span>
          </li>
          <li>
            <el-tag type="primary" effect="plain">历史数据分析</el-tag>
            <span>可视化展示历史真实流量和历史预测流量对比</span>
          </li>
          <li>
            <el-tag type="primary" effect="plain">实时预测展示</el-tag>
            <span>同时显示历史值与未来预测值,支持灵活的时间窗口选择</span>
          </li>
          <li>
            <el-tag type="primary" effect="plain">资源信息监控</el-tag>
            <span>提供资源配置和状态的真实观测信息展示</span>
          </li>
        </ul>
      </div>
...
export default {
  ...
  data() {
    return {
      externalUrl: "http://82.157.104.186:9528/",
    };
  },
  computed: {
  ...
  activated() {
    console.log("✅ [TrafficPrediction] activated()");
    // 组件激活时,如果之前显示了iframe,继续显示
  },
  deactivated() {
    console.log("⏸️ [TrafficPrediction] deactivated()");
    // 组件失活时,不修改showIframe状态,保持iframe的显示状态
    // GlobalIframeManager会根据当前路由自动控制显示/隐藏
  },
  methods: {
    enterSystem() {
      console.log("🚀 [TrafficPrediction] 进入iframe模式");
      ...
      console.log("✅ [TrafficPrediction] 状态设置完成");
    },
    goBack() {
  ...
  .module-card {
    ...
    .module-header {
      ...
      .module-icon {
        ...
        margin-bottom: 25px;
        background: linear-gradient(135deg, #6B46C1 0%, #9333EA 50%, #A78BFA 100%);
        box-shadow:
