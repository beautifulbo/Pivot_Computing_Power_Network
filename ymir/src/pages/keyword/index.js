...
function Keyword({ getKeywords }) {
  ...
  async function getData() {
    ...
    // === Mock 替换 L111-115 开始 ===
    const enableMock = process.env.USE_MOCK === 'true' || (process.env.NODE_ENV === 'development' && !getKeywords)
    let items, total
    if (enableMock) {
      // 模拟网络延迟
      await new Promise((r) => setTimeout(r, 300))
      const mockItems = [
        { name: 'dog', aliases: ['puppy', 'canine'], update_time: Date.now(), create_time: Date.now() },
        { name: 'cat', aliases: ['kitty', 'feline'], update_time: Date.now(), create_time: Date.now() },
        { name: 'car', aliases: ['automobile', 'vehicle'], update_time: Date.now(), create_time: Date.now() },
      ]
      // 简单的查询过滤
      const filtered = params.q ? mockItems.filter(i => i.name.toLowerCase().includes(params.q)) : mockItems
