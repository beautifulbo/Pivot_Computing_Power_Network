...
const Datasets: ModuleType = ({ pid, project, iterations, groups }) => {
  ...
  const [datasetList, setdatasetList] = useState({items:{},total:0})
  const [versions, setversions] = useState<IdMap<Dataset[]>>({})
  ...
  // const { data:res, run: getdatasetstate } = useRequest<any, [{ pid: number}]>('dataset/getDatasetGroups')
  ...
  const fetchData = async () => {
    const res = await getdatasetstate(pid)
    if(res.code == 0){
        return res.data
    }
    else{
  ...
  const columns = (gid: number,datasetname:string,source:string,createTime:string): TableColumnsType<Dataset> => {
    return [
      {
        ...
        render: (name, record) => {
          ...
          const content = (
            <Row key={id}>
              <Col flex={1}>{readyState(state) ? name : datasetname}</Col>
      ...
      // },
      {
        ...
        render:(record) => {return(<Col flex={1}>{createTime}</Col>)},
          ...
          <div className={styles.groupItem} key={group.id}>
            <Row className="groupTitle">
              <Col flex={1} onClick={() => toggleVersions(group.id)}>
                ...
                <span className="groupName">数据集{group.id}</span>
            ...
            <div className="groupTable" hidden={visibles[group.id] === false}>
              <Table
                ...
                columns={columns(group.id,group.datasetname ,group.source,group.createtime)}
                pagination={false}
              />
